...
function microsoftRequestFailure(response, stage) {
    if(response.microsoftErrorCode !== MicrosoftErrorCode.UNKNOWN) {
        return microsoftErrorDisplayable(response.microsoftErrorCode)
    }

    const status = response.error?.response?.statusCode
    const body = response.error?.response?.body
    const message = typeof body === 'string' ? body : body?.errorMessage || body?.error_description || ''
    if(stage === 'Minecraft 인증' && status === 403 && /invalid app registration/i.test(message)) {
        return {
            title: 'Minecraft 앱 승인 필요',
            desc: 'Minecraft 서비스가 이 런처의 앱 등록을 거부했습니다 (HTTP 403). 운영자의 앱 ID에 대한 별도 승인이 필요합니다.'
        }
    }

    const rawCode = typeof body === 'object' ? body?.error : undefined
    const safeCode = typeof rawCode === 'string' && /^[A-Za-z0-9_.-]{1,60}$/.test(rawCode) ? ` / ${rawCode}` : ''
    const statusText = Number.isInteger(status) ? `HTTP ${status}${safeCode}` : '연결 또는 응답 오류'
    return {
        title: `Microsoft 로그인 실패: ${stage}`,
        desc: `${stage} 단계에서 ${statusText}가 발생했습니다. 이 화면을 운영자에게 알려주세요.`
    }
}
    ...
    try {
        ...
        if(authMode !== AUTH_MODE.MC_REFRESH) {
            ...
            if(accessTokenResponse.responseStatus === RestResponseStatus.ERROR) {
                return Promise.reject(microsoftRequestFailure(accessTokenResponse, 'Microsoft 토큰'))
            }
        ...
        if(xblResponse.responseStatus === RestResponseStatus.ERROR) {
            return Promise.reject(microsoftRequestFailure(xblResponse, 'Xbox 로그인'))
        }
        const xstsResonse = await MicrosoftAuth.getXSTSToken(xblResponse.data)
        if(xstsResonse.responseStatus === RestResponseStatus.ERROR) {
            return Promise.reject(microsoftRequestFailure(xstsResonse, 'Xbox 보안 토큰'))
        }
        const mcTokenResponse = await MicrosoftAuth.getMCAccessToken(xstsResonse.data)
        if(mcTokenResponse.responseStatus === RestResponseStatus.ERROR) {
            return Promise.reject(microsoftRequestFailure(mcTokenResponse, 'Minecraft 인증'))
        }
        const mcProfileResponse = await MicrosoftAuth.getMCProfile(mcTokenResponse.data.access_token)
        if(mcProfileResponse.responseStatus === RestResponseStatus.ERROR) {
            return Promise.reject(microsoftRequestFailure(mcProfileResponse, 'Minecraft 프로필'))
        }
        return {
            accessToken,
            accessTokenRaw,
