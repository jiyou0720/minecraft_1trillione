...
public class ShopAdminCommand extends CommandBase {
    ...
    @Override public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        ...
        if ((args[0].equals("상품") || args[0].equals("products")) && args.length == 2) {
            ...
            for (int i = 0; i < shop.products.size(); i++) {
                ...
                sender.sendMessage(Texts.text("&e#" + (i + 1) + " " + p.item.getDisplayName() +
                        ...
                        " / 최대 " + (p.sellEnabled ? Texts.money(p.maxSellPrice) : "해당없음") + " / 연속 하락 " + p.consecutiveDrops));
            }
            return;
        }
        if ((args[0].equals("최대가") || args[0].equals("maxprice")) && args.length == 4) {
            ...
            long cap = parseLong(args[3], 1, Long.MAX_VALUE);
            if (cap < product.minSellPrice) throw new CommandException("최대판매가는 최저판매가 이상이어야 합니다.");
            product.maxSellPrice = cap;
            if (product.currentSellPrice > cap) product.currentSellPrice = cap;
            product.previousSellPrice = product.currentSellPrice;
            data.markDirty();
            sender.sendMessage(Texts.text("&a상품 #" + (index + 1) + " 최대 판매가: " + Texts.money(cap)));
            return;
        }
        if ((args[0].equals("등록") || args[0].equals("register")) && (args.length == 6 || args.length == 7)) {
            ...
            if (sell && (args.length != 7 || specifiedCap < sellPrice))
                throw new CommandException("판매 상품은 최대판매가를 반드시 입력해야 하며 최초 판매가 이상이어야 합니다.");
            if (!sell && args.length == 7 && specifiedCap != 0)
                throw new CommandException("판매하지 않는 상품의 최대판매가는 0으로 입력해 주세요.");
            ShopData.Product product = new ShopData.Product();
